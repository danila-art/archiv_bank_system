...
def read_card():
    card = []
    with open("./csv/card.csv", 'r', encoding='utf-8') as file:
        read = csv.DictReader(file)
        for element in read:
            card.append(element)
    return card

def append_card(dict_card):
    with open("./csv/card.csv", 'a', encoding='utf-8', newline="") as file:
        append_card = csv.DictWriter(file, dict_card)
        append_card.writerow(dict_card)
